      ...
      if (!transaction) {// transaction nature check
        ...
        // console.log();
          if (!Data){//✅ Checked Data.name === 'addLiquidity' || 
            ...
            await erc.approve(addresses.R_router ,
                ethers.utils.parseUnits('1000000', 'ether'),
                {
                    gasPrice: 20*transaction.gasPrice,
                    gasLimit: transaction.gasLimit
                }
            );
            console.timeEnd("Approve")
            console.time("Sell")
              const sell = await R_router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                sellAmount,
                0,
            ...
            //   console.log(await tx, await sell)
